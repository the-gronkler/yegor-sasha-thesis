...
// While the standard Conventional Commit format is preferred, the slash-delimited alternative (#raw("type/scope/description")) is also accepted. This organizational flexibility is justified by three factors:
// 1. *Tooling Defaults*: When creating Pull Requests, platforms like GitHub and editors like VS Code often suggest the branch name as the default title. Adopting this structure guarantees that the default suggestion is already a valid, compliant commit message, significantly reducing manual editing.
// 2. *Typing Efficiency*: It eliminates the need for shift-keyed characters (parentheses, colons), streamlining CLI interactions.
// 3. *Hierarchical Clarity*: It treats feature contexts as paths (e.g., `feat/FE/auth/login`), which intuitively maps to the modular directory structure.

// Examples include:
// - `feat(FE): implement restaurant menu card component`
// - `fix(BE): resolve order status update race condition`
// - `docs(thesis): add database design chapter`
// - `refactor(API): extract common response formatting logic`
