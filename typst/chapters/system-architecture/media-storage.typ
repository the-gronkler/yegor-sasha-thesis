...
To manage the metadata associated with creating and displaying images, the system implements a unified `Image` entity#footnote[#source_code_link("app/Models/Image.php")]. A dedicated `images` table serves as a central registry for all media assets, linked to restaurants via a `restaurant_id` foreign key. Menu items can optionally reference a specific image through an `image_id` foreign key on the `menu_items` table.
